...
public class MultiGaussianDistribution implements MultiRandomDistribution {
     ...
     *
     * @param mean
     *            The mean vector of the generated numbers. This array is
     *            copied.
     * @param covariance
     *            The covariance of the generated numbers. This array is copied.
     *            <code>covariance[r][c]</code> is the element at row
    ...
    public double probability(double[] v) {
        ...
        double expArg = (SimpleMatrix.times(SimpleMatrix.transpose(vmm), SimpleMatrix.times(covarianceInv(), vmm))[0][0])
                * -.5;

        return Math.exp(expArg) / (Math.pow(2. * Math.PI, ((double) dimension) / 2.) * Math.pow(covarianceDet(), .5));
    }
