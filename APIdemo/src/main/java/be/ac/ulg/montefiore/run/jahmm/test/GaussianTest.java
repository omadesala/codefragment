...
public class GaussianTest extends TestCase {
    ...
    public void testGaussianFit() {
        ...
        OpdfMultiGaussian omg2 = new OpdfMultiGaussian(new double[] { 0., 0. },
                new double[][] { { 0., 0. }, { 0., 0. } });
        ...
        assertTrue("Different mean arrays: " + toString(mean) + " differ from " + toString(omg2.mean()),
                equalsArrays(mean, omg2.mean()));

        for (int i = 0; i < 2; i++)
            assertTrue("Different covariance arrays: " + toString(omg1.covariance()[i]) + " differ from "
                    + toString(omg2.covariance()[i]),
                    equalsArrays(omg1.covariance()[i], omg2.covariance()[i], DELTA * 10.));
    }

    public void testGaussianMixture() {
        /*
         * Generates observations related to 2 gaussians : (0., 1.) and (4.,
         * 2.). Proportions : 1/3, 2/3
         */
        ObservationReal[] observations = new ObservationReal[nbObservations];

        for (int g = 0, i = 0; g < 2; g++) {
            RandomDistribution d = new GaussianDistribution(g * 4., 1 + g);
        ...
        assertTrue("Wrong proportion values (" + gm.proportions()[0] + ", " + gm.proportions()[1] + ")",
                equalsArrays(new double[] { 1. / 3., 2. / 3. }, gm.proportions(), DELTA));

        assertTrue("Wrong mean values (" + gm.means()[0] + ", " + gm.means()[1] + ")",
                equalsArrays(new double[] { 0., 4. }, gm.means(), DELTA * 10.));
        assertTrue("Wrong variance values (" + gm.variances()[0] + ", " + gm.variances()[1] + ")",
                equalsArrays(new double[] { 1., 2. }, gm.variances(), DELTA * 10.));
    }
    ...
    public void testMultiGaussianFit() {
        ...
        for (int i = 0; i < 2; i++)
            assertTrue("Different covariance arrays", equalsArrays(omg1.covariance()[i], omg2.covariance()[i]));
    }
