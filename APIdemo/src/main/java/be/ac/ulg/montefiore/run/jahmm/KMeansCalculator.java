 ...
 * <i>Clustering and the Continuous k-Means Algorithm</i> (Vance Faber, <i>Los
 * Alamos Science</i> number 22).
 * <p>
...
public class KMeansCalculator<K extends CentroidFactory<? super K>> {
    ...
    class Cluster<L extends CentroidFactory<? super L>> {
         ...
         *
         * @param element
         *            The element that compose the new cluster.
         */
        public Cluster(L e) {
            elements = new ArrayList<L>();
            elements.add(e);
     ...
     * @param elements
     *            The elements to divide in clusters.
     */
    public KMeansCalculator(int k, List<? extends K> elements) {
        ...
        elLoop: for (; elementNb < elements.size() && clusterNb < k && elements.size() - elementNb > k - clusterNb; elementNb++) {
        ...
        for (; clusterNb < k && elementNb < elements.size(); elementNb++, clusterNb++)
            clusters.add(new Cluster<K>(elements.get(elementNb)));
        ...
        do {
            ...
            for (int i = 0; i < k; i++) {
                ...
                for (int j = 0; j < thisElements.size(); j++) {
                    ...
                    /*
                     * We don't move an element if it is the only one in its
                     * cluster.
                     */
