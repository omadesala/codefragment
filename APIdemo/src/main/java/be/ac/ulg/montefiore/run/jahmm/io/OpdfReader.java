...
public abstract class OpdfReader<O extends Opdf<?>> {
     ...
     * <p>
     * The stream tokenizer syntax table must be set according to of
     * <code>HmmReader.initSyntaxTable(StreamTokenizer)
     ...
     * 
     * @param st
     *            The tokenizer to read the sequence from.
     * @param length
     *            The expected length of the sequence or a strictly negative
     *            number if it must not be checked.
     * @return The array read.
     */
    static protected double[] read(StreamTokenizer st, int length) throws IOException, FileFormatException {
        ...
        if (length >= 0 && l.size() != length)
            throw new FileFormatException(st.lineno(), "Wrong length of number sequence");

        if (l.size() == 0)
            throw new FileFormatException(st.lineno(), "Invalid empty sequence");
