...
public class CosineDistanceMeasure extends EuclideanDistance {
    ...
    /**
     * returns the cosine distance. To find the actual angle you need to take
    ...
    public double distance(Instance inst1, Instance inst2) {
        ...
        for (int count = 0; count < inst1.numAttributes(); count++) {
            ...
            inst1magnitude = inst1magnitude
                    + (inst1.value(att1) * inst1.value(att1));

            inst2magnitude = inst2magnitude
                    + (inst2.value(att2) * inst2.value(att2));
        }

        inst1magnitude = Math.sqrt(inst1magnitude);
        inst2magnitude = Math.sqrt(inst2magnitude);

        if ((inst1magnitude * inst2magnitude) == new Double("0.000000000000")
                .doubleValue())
            return 0;
...
//        return value > arg2 ? Double.POSITIVE_INFINITY : value;
    ...
    public void clean() {
        ...
        // super.clean();
        // this.data = null;
        m_Data = new Instances(this.data, 0);
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        Instances load = WekaUtil.load("IBM11");

        CosineDistanceMeasure cd = new CosineDistanceMeasure();
        cd.setInstances(load);

        double distance = cd.distance(load.instance(41), load.instance(112));
        System.out.println("the dist: " + distance);
        // System.out.println(cd.distance(load.instance(2), load.instance(3)));
    }
}