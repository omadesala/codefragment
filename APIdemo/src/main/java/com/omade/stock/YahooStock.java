...
public class YahooStock {
	...
	public static void main(String[] args) {
		...
		do {
			...
			stockCsvData = stockUtil.getStockCsvData("VIPS", parseDate2Str,
					today);
			if (loopcount > 0) {
				try {
					Thread.sleep(1000);
	...
	public List<StockData> readData(String tableName, Date from, int length) {
		List<StockData> records = storage.getRecord(tableName,
				Utils.parseStr2Date("2013-07-26"), 3);
