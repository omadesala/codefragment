...
//        Matrix data = MatrixUtils.randomRow(10);
        Matrix data = Matrix.random(10, 10);
//        MatrixUtils.printMatrix(data);
//        showData("stock data", "time", "price", data);
        showDataArray("stock data", "time", "price", data);
    }

    /**
     * @param data one dim data
     */
     ...
     * @param data one dim data
     */
    public static void showDataArray(String title, String xAxisName,
                                     String yAxisName, Matrix data) {
        ...
        StringBuffer xrange = new StringBuffer();
        int columnsize = data.getColumnDimension();
        int rows = data.getRowDimension();

        xrange.append("set xrange [0:" + columnsize + "]");
        ...
        for (int j = 0; j < rows; j++) {
            ...
            double[] rowData = MatrixUtils.getRowAsArray(data, j);
            Arrays.sort(rowData);
            Matrix points = new Matrix(2, columnsize);

            for (int i = 0; i < columnsize; i++) {
                points.set(0, i, i);
                points.set(1, i, rowData[i]);
            }
            MatrixUtils.printMatrix(points);
